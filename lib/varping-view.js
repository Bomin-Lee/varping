...
export default class VarpingView {
  ...
  constructor(serializedState) {
    ...
    inputArea.changeElement = function() {
      // view.clearAllTags();

      view.addStrings(getRelatedStrings(inputArea.value, targets));
      console.log(targets);
      if (view.addedElements.length > 0){
        for(var i = 0; i < view.addedElements.length; i++) {
          window.a = view.addedElements
          var isFind = false;
          for(var j = 0; j < view.addedElements[i].classList.length; j++) {
            if(view.addedElements[i].classList[j] == "varping_active"){
              view.addedElements[i].classList.remove("varping_active")
              // view.addedElements[i].classList.removeChild(view.addedElements[i].classList[j]);
              isFind = true;
              break;
             }
          }
    ...
    inputArea.oninput = function() {
      ...
      request('http://52.197.175.145:5000/ddtest/?pattern=' + inputArea.value,
      function(error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
          ...
          for (let i = 0; i < obj.recommendations.length; i++) {
            targets.push(obj.recommendations[i]);
          }
